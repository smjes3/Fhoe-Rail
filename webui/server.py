...
端点:
    ...
    GET  /api/map?file=&version= -> 读取单个地图 JSON
    POST /api/map            -> 保存地图 JSON（请求体 {file, version, data}）
    POST /api/open-map       -> 用系统默认程序打开地图 JSON 文件（请求体 {file, version}）
...
def _backup_file(path):
    """原子写入前自动备份，保留最近 3 份 .bakN"""
    if not os.path.exists(path):
        return
    # 轮转: bak2 -> bak3, bak1 -> bak2, 原文件 -> bak1
...
def run_start(mode, map_id=None, map_version=None):
    """启动锄地进程（单例：已有进程时拒绝）
    mode: normal/white/record/install/map/devmap
    map_id: 仅 map/devmap 模式需要，如 '3-5_1'
    map_version: 仅 map/devmap 模式可选，如 'default' / 'HuangQuan'
    ...
    with _RUN["lock"]:
        ...
        if mode in ("map", "devmap"):
            ...
            base_args = (
                ["webui/launch.py", "--dev", "--map", map_id]
                if mode == "devmap"
                else ["webui/launch.py", "--map", map_id]
            )
            if map_version:
                base_args += ["--map-version", map_version]
            args = base_args
        ...
        if os.path.exists(FHOE_EXE):
            ...
            if mode == "install":
                return {
                    "ok": False,
                    "error": "打包版无需安装依赖，请直接双击 Fhoe-Rail.exe 运行",
                }
            exe_args = [FHOE_EXE]
            if mode == "white":
                ...
                exe_args += ["--map", map_id]
                if map_version:
                    exe_args += ["--map-version", map_version]
            elif mode == "devmap":
        ...
        try:
            ...
            if getattr(e, "winerror", None) == 740:
                ...
                return {
                    "ok": False,
                    "error": "Fhoe-Rail.exe 需要管理员权限(旧版打包)。请重新下载不带提权的新版,或右键“以管理员身份运行”本窗口。",
                }
...
def parse_log_stats(tail_lines):
    ...
    for key, pat in [
        ...
        (
            "fight_time",
            r"总计战斗用时 ([\d.]+分[\d.]+秒|[\d.]+秒|[\d.]+小时[\d.]+分[\d.]+秒)",
        ),
        (
            "save_time",
            r"疾跑节约的时间为 ([\d.]+分[\d.]+秒|[\d.]+秒|[\d.]+小时[\d.]+分[\d.]+秒)",
        ),
        ...
        (
            "total_time",
            r"总计用时 ([\d.]+分[\d.]+秒|[\d.]+秒|[\d.]+小时[\d.]+分[\d.]+秒)",
        ),
    ]:
        ...
        try:
            if path == "/" or path == "/index.html":
                ...
                if not _cfg.get("version"):
                    ...
                    try:
                        with open(
                            os.path.join(BASE_DIR, "version.txt"), "r", encoding="utf-8"
                        ) as _f:
                            _v = _f.read().strip()
                        if _v:
                            _cfg["version"] = _v
                ...
                name = qs.get("name", [""])[0]
                if not re.match(
                    r"^[\u4e00-\u9fff\w\-, ]+\.(png|jpg|jpeg|webp|gif)$", name, re.I
                ):
                ...
                if not p or "/" in p or "\\" in p or ".." in p:
                    ...
                    return
                if not re.match(r"^[\w\-]+\.(png|jpg|jpeg|webp|gif)$", p, re.I):
                    self._send(400, {"error": "bad path"})
                    return
                base = p.rsplit(".", 1)[0]
                found = None
                for ext in ("webp", "png", "jpg", "jpeg"):
                    cand = os.path.join(BASE_DIR, "picture", base + "." + ext)
                    if os.path.exists(cand):
                        found = cand
                        break
                if not found:
                ...
                self._send(
                    200,
                    {
                        ...
                        "map_versions": {
                            k: sum(len(v) for v in ver.values())
                            for k, ver in tree.items()
                        },
                        "log_stats": stats,
                        "planet_names": PLANET_NAMES,
                        "version_names": VERSION_NAMES,
    ...
    def do_POST(self):
        ...
        if parsed.path == "/api/config":
            ...
            try:
                ...
                self._send(
                    200, run_start(mode, data.get("map"), data.get("map_version"))
                )
            except Exception as e:
                self._send(400, {"ok": False, "error": str(e)})
            ...
            try:
                ...
                data = body.get("data", {})
                # 路径安全校验：只允许写入 map 目录下的合法地图 JSON
                if not re.match(r"^[A-Za-z0-9_]+$", version):
                    self._send(400, {"ok": False, "error": "bad version"})
                    return
                if not re.match(r"^map_\d+-\d+_\d+\.json$", file):
                    self._send(400, {"ok": False, "error": "bad file"})
                    return
                fpath = os.path.join(BASE_DIR, "map", version, file)
                if not os.path.exists(fpath):
                    self._send(404, {"ok": False, "error": "map not found"})
                    return
                # 校验 data 结构
                if not isinstance(data, dict):
                    raise ValueError("data 必须是 JSON 对象")
                for key in ("name", "author", "start", "map"):
                    if key not in data:
                        raise ValueError(f"缺少必要字段: {key}")
                if not isinstance(data["start"], list) or not isinstance(
                    data["map"], list
                ):
                    raise ValueError("start 和 map 必须是数组")
                _backup_file(fpath)
                tmp = fpath + ".tmp"
                with open(tmp, "w", encoding="utf-8") as f:
            ...
            try:
                ...
                self._send(
                    ...
                    {
                        "ok": ok,
                        "note": "通知已发送，请检查手机/客户端"
                        if ok
                        else "发送失败，请检查配置（总开关、渠道、密钥）",
                    },
                )
            except Exception as e:
